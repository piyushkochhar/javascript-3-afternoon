...
//Code Here
let evenNumbers  = mixedNumbers.filter(numbers => numbers%2 === 0)
...
//Code Here
let postTaxPrices  = prices.map(number => number + (number*0.07));
...
let bobsTotal = purchases.filter(person => person.owner === 'Bob').reduce((sum,next) => (sum + next.price),0)
